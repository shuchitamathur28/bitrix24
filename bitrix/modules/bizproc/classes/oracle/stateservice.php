	...
	extends CBPAllStateService
{
	public function SetState($workflowId, $arState, $arStatePermissions = array())
	{
		...
		$strUpdate = $DB->PrepareUpdate("b_bp_workflow_state", array(
			"STATE" => (strlen($state) > 0 ? $state : false),
			"STATE_TITLE" => (strlen($stateTitle) > 0 ? $stateTitle : false),
			"STATE_PARAMETERS" => $stateParameters,
			"~MODIFIED" => $DB->CurrentTimeFunction(),
		));
		$arBinds = array("STATE_PARAMETERS" => $stateParameters);
		$DB->QueryBind("update b_bp_workflow_state set ".$strUpdate." WHERE ID = '".$DB->ForSql($workflowId)."'", $arBinds);
	...
	public function SetStateParameters($workflowId, $arStateParameters = array())
	{
		...
		$strSql = "UPDATE b_bp_workflow_state SET ".$strUpdate." WHERE ID = '".$DB->ForSql($workflowId)."' ";
		$arBinds = array("STATE_PARAMETERS" => $stateParameters);
	...
	public static function getWorkflowIntegerId($workflowId)
	{
		...
		if (!$result)
		{
			$nextId = (int)$DB->NextID("S_B_BP_WORKFLOW_STATE_IDENTIFY");
			$strSql =
				"INSERT INTO b_bp_workflow_state_identify (ID, WORKFLOW_ID) ".
				"VALUES (".$nextId.", '".$DB->ForSql($workflowId)."')";
			$DB->Query($strSql);
