...
class CIBlockProperty extends CAllIBlockProperty
{
			...
			{//MULTIPLE=N
				...
				$strSql = "
					...
					SET
						...
						".(isset($tableFields["DESCRIPTION_".$ID])? ",DESCRIPTION_".$ID." = EN.DESCRIPTION": "")."
					FROM
		...
		{//WITH_DESCRIPTION changed
			...
			{//WITH_DESCRIPTION=Y
				if (!isset($tableFields["DESCRIPTION_".$ID]))
				{
					...
					if (!$DB->DDL($strSql))
					{
						$this->LAST_ERROR = $this->FormatUpdateError($ID, "MS14");
						return false;
					}
				}
			}
			...
			{//WITH_DESCRIPTION=N
				if (isset($tableFields["DESCRIPTION_".$ID]))
				{
					...
					if ($ar["CNT"] <= 0)
					{
						$strSql = "
							ALTER TABLE b_iblock_element_prop_s".$arProperty["IBLOCK_ID"]."
							DROP COLUMN DESCRIPTION_".$ID."
						";
						if (!$DB->DDL($strSql))
						{
							$this->LAST_ERROR = $this->FormatUpdateError($ID, "MS15");
							return false;
						}
					}
				}
	...
	function DropColumnSQL($strTable, $arColumns)
	{
		...
		foreach ($arColumns as $i => $columnName)
		{
			if (isset($tableFields[$columnName]))
				$res[]="ALTER TABLE ".$strTable." DROP COLUMN ".$columnName;
		}
		return $res;
	}
	...
	function _Add($ID, $arFields)
	{
		...
		$strSql = "
			...
			".($arFields["WITH_DESCRIPTION"] == "Y"? ", DESCRIPTION_".$ID." varchar(255)": "")."
		";
