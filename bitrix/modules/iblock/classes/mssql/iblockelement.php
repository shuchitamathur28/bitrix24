	...
	function prepareSql($arSelectFields=array(), $arFilter=array(), $arGroupBy=false, $arOrder=array("SORT"=>"ASC"))
	{
		...
		$arIblockElementFields = array(
				...
				"ACTIVE_FROM"=>"BE.ACTIVE_FROM as AF_TMP, ".(
						$formatActiveDates
						?
							"BE.ACTIVE_FROM, ".$DB->DateToCharFunction("BE.ACTIVE_FROM", $shortFormatActiveDates)
						:
							"
								BE.ACTIVE_FROM, case
									when (
				...
				"ACTIVE_TO"=>(
						$formatActiveDates
						?
							"BE.ACTIVE_TO, ".$DB->DateToCharFunction("BE.ACTIVE_TO", $shortFormatActiveDates)
						:
							"
								BE.ACTIVE_TO, case
									when (
				...
				"DATE_ACTIVE_FROM"=>(
						$formatActiveDates
						?
							"BE.ACTIVE_FROM DATE_ACTIVE_FROM, ".$DB->DateToCharFunction("BE.ACTIVE_FROM", $shortFormatActiveDates)
						:
							"
				...
				"DATE_ACTIVE_TO"=>(
						$formatActiveDates
						?
							"BE.ACTIVE_TO DATE_ACTIVE_TO, ".$DB->DateToCharFunction("BE.ACTIVE_TO", $shortFormatActiveDates)
						:
							"
	...
	public static function GetList($arOrder=array("SORT"=>"ASC"), $arFilter=array(), $arGroupBy=false, $arNavStartParams=false, $arSelectFields=array())
	{
		...
		if(!empty($arNavStartParams) && is_array($arNavStartParams))
		{
			...
			if($nTopCount > 0)
			{
				...
				$res = $DB->Query($strSql.$el->sOrderBy, false, "FILE: ".__FILE__."<br> LINE: ".__LINE__);
			}
			elseif(
				$nElementID > 0
				&& $el->sGroupBy == ""
			...
			)
			{
				...
				$strRankSql = preg_replace(
					...
					"SELECT RANK() OVER (".$el->sOrderBy.") AS RANK, \\2",
					str_replace(
						"E.TIMESTAMP_X,",
						"",
						str_replace(
		...
		foreach ($ar_prop as $prop)
		{
			...
			elseif ($prop["PROPERTY_TYPE"] == "F")
			{
				...
				foreach ($orderedPROP as $propertyValueId => $val)
				{
					...
					{
						$DB->Query("SET IDENTITY_INSERT $strTable ON");
						$DB->Query("
							INSERT INTO ".$strTable."
							...
							WHERE
								ID = ".IntVal($prop["ID"])."
						");
						$DB->Query("SET IDENTITY_INSERT $strTable OFF");
					}
					else
					{
						$DB->Query("
