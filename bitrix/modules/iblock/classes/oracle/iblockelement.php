...
{
	public static function array_compare($arr1, $arr2, $bIgnoreCase = false)
	{
	...
	public static function __GetAddIndexes()
	{
		$arrIndexes = Array();
	...
	public static function GetList($arOrder=array("SORT"=>"ASC"), $arFilter=array(), $arGroupBy=false, $arNavStartParams=false, $arSelectFields=array())
	{
		...
		if(!empty($arNavStartParams) && is_array($arNavStartParams))
		{
			...
			)
			{
				...
				foreach(explode(",", $el->sSelect) as $str)
				{
					$str = trim($str);
					list($columnName, $columnAlias) = explode(" as ", $str);
					$selectedColumns[$columnName] = $columnAlias;
				}

				$sOrderBy = "";
				$arSqlOrder = explode(",", preg_replace("/^(\\s*ORDER\\s+BY\\s+)/i", "", $el->sOrderBy));
				foreach($arSqlOrder as $i=>$val)
				{
					$val = trim($val);
					if(strlen($val) > 0 && preg_match("/^(.*?)\\s+(asc.*|desc.*)\$/is", $val, $orderMatch))
					{
						if (isset($selectedColumns[$orderMatch[1]]))
						{
							if($sOrderBy=="")
								$sOrderBy = " ORDER BY ";
							else
							...
							$sOrderBy .= $selectedColumns[$orderMatch[1]];
						}
					}
				}
	...
	function Update($ID, $arFields, $bWorkFlow=false, $bUpdateSearch=true, $bResizePictures=false, $bCheckDiskQuota=true)
	{
		...
		else
		{
			...
			if(!empty($strUpdate) && $bTimeStampNA)
				$strUpdate .= ", ";
			...
			if(!empty($strUpdate) || $bTimeStampNA)
			{
				...
				if (is_array($SAVED_DETAIL_PICTURE) && $SAVED_DETAIL_PICTURE["old_file"] > 0)
				{
					CIBlockElement::DeleteFile(
						$SAVED_DETAIL_PICTURE["old_file"],
						false,
						"DETAIL",
						$ID,
						$arIBlock["ID"]
				...
				if (is_array($SAVED_PREVIEW_PICTURE) && $SAVED_PREVIEW_PICTURE["old_file"] > 0)
				{
					CIBlockElement::DeleteFile(
						$SAVED_PREVIEW_PICTURE["old_file"],
						false,
						"PREVIEW",
						$ID,
						$arIBlock["ID"]
