	...
	*/
	function DropTemporaryTables()
	{
		...
		else
		{
			...
			$DB->DDL("drop sequence sq_".$this->_table_name."", true);
			if($DB->TableExists($this->_table_name))
	...
	function CreateTemporaryTables($with_sess_id = false)
	{
		...
		else
		{
			...
			if($res)
				$res = $DB->DDL("CREATE SEQUENCE sq_".$this->_table_name);
			if($res)
				$res = $DB->DDL("CREATE OR REPLACE TRIGGER ".$this->_table_name."_insert
BEFORE INSERT
ON ".$this->_table_name."
FOR EACH ROW
BEGIN
