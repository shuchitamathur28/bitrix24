...
class CIBlockProperty extends CAllIBlockProperty
{
		...
		{//MULTIPLE changed
			...
			{//MULTIPLE=N
				...
				$strSql = "
					...
					SET
						(PROPERTY_".$ID."
						".(isset($tableFields["DESCRIPTION_".$ID])? ",DESCRIPTION_".$ID: "")."
						) = (
							SELECT
								".$strTrans."
								".(isset($tableFields["DESCRIPTION_".$ID])? ",DESCRIPTION": "")."
		...
		{//WITH_DESCRIPTION changed
			...
			{//WITH_DESCRIPTION=Y
				if (!isset($tableFields["DESCRIPTION_".$ID]))
				{
					$strSql = "
						ALTER TABLE b_iblock_element_prop_s".$arProperty["IBLOCK_ID"]."
						ADD (DESCRIPTION_".$ID." VARCHAR2(255 CHAR))
					";
					if (!$DB->DDL($strSql))
					{
						$this->LAST_ERROR = $this->FormatUpdateError($ID, "OR13");
						return false;
					}
				}
			}
			...
			{//WITH_DESCRIPTION=N
				if (isset($tableFields["DESCRIPTION_".$ID]))
				{
					$rs = $DB->Query("
						...
						WHERE DESCRIPTION_".$ID." IS NOT NULL
					");
					$ar = $rs->Fetch();
					if ($ar["CNT"] <= 0)
					{
						...
						if (!$DB->DDL($strSql))
						{
							$this->LAST_ERROR = $this->FormatUpdateError($ID, "OR14");
							return false;
						}
					}
				}
	...
	function _Add($ID, $arFields)
	{
		...
		$strSql = "
			...
			".($arFields["WITH_DESCRIPTION"] == "Y"? ", DESCRIPTION_".$ID." VARCHAR2(255 CHAR)": "").")
		";
