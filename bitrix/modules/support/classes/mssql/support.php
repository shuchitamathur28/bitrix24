	...
	public static function AutoClose()
	{
		...
		$strSql = "
			...
			WHERE
				T.AUTO_CLOSE_DAYS > 0
			and T.DATE_CLOSE is null
			and datediff(day, T.LAST_MESSAGE_DATE, $nowTime) > T.AUTO_CLOSE_DAYS
	...
	public static function GetList(&$by, &$order, $arFilter=Array(), &$isFiltered, $checkRights="Y", $getUserName="Y", $getExtraNames="Y", $siteID=false, $arParams = Array() )
	{
			...
			$lamp = "
				case
					when T.DATE_CLOSE is not null then 'grey'
						...
						case
							when T.LAST_MESSAGE_BY_SUPPORT_TEAM='Y' then 'red'
							else 'green'
						end
				end
				";
			...
			for ($i=0; $i<$filterKeysCount; $i++)
			{
				...
				switch($key)
				{
					...
					case "AUTO_CLOSE_DAYS_LEFT1":
						$arSqlSearch[] = "CASE WHEN T.DATE_CLOSE IS NULL AND T.LAST_MESSAGE_BY_SUPPORT_TEAM = 'Y' THEN
							datediff(day, getdate(), dateadd(day, T.AUTO_CLOSE_DAYS, T.LAST_MESSAGE_DATE)) ELSE -1 END >= '".intval($val)."'
							";
						break;
					case "AUTO_CLOSE_DAYS_LEFT2":
						$arSqlSearch[] = "CASE WHEN T.DATE_CLOSE IS NULL AND T.LAST_MESSAGE_BY_SUPPORT_TEAM = 'Y' THEN
							datediff(day, getdate(), dateadd(day, T.AUTO_CLOSE_DAYS, T.LAST_MESSAGE_DATE)) ELSE 999 END <= '".intval($val)."'
							";
						break;
					case "OWNER":
						$getUserName = "Y";
		...
		if (strlen($siteID)>0)
		{
			$dates_select = "
				...
				CASE WHEN T.DATE_CLOSE IS NULL AND T.LAST_MESSAGE_BY_SUPPORT_TEAM = 'Y' THEN ".$DB->DateToCharFunction("
					dateadd(day, T.AUTO_CLOSE_DAYS, T.LAST_MESSAGE_DATE)","FULL",$siteID,true)." ELSE NULL END	AUTO_CLOSE_DATE
			";
		}
		else
		{
		...
		$strSqlSelect = "
			SELECT
				...
				CASE WHEN T.DATE_CLOSE IS NULL AND T.LAST_MESSAGE_BY_SUPPORT_TEAM = 'Y' THEN
					datediff(day, getdate(), dateadd(day, T.AUTO_CLOSE_DAYS, T.LAST_MESSAGE_DATE)) ELSE -1 END	AUTO_CLOSE_DAYS_LEFT,
				(SELECT COUNT(DISTINCT USER_ID) FROM b_ticket_online WHERE TICKET_ID = T.ID AND TIMESTAMP_X >= dateadd(second, - $onlineInterval, getdate()))	USERS_ONLINE,
				case
