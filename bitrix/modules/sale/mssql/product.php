...
class CSaleProduct extends CALLSaleProduct
{
	...
	function GetProductList($ID, $minCNT, $limit, $getParentOnly = false)
	{
		...
		if ($getParentOnly)
		{
			$strSql = ($limit > 0 ? 'select top '.$limit : 'select')." PARENT_PRODUCT_ID from b_sale_product2product where PRODUCT_ID in (".implode(',', $elementInclude).")";
		...
		{
			$strSql = ($limit > 0 ? 'select top '.$limit : 'select')." * from b_sale_product2product where PRODUCT_ID in (".implode(',', $elementInclude).")";
			if (!empty($elementExclude))
				$strSql .= " and PARENT_PRODUCT_ID not in (".implode(',', $elementExclude).")";
			if ($minCNT > 0)
				$strSql .= " and CNT >= ".$minCNT;
	...
	function GetBestSellerList($by = "AMOUNT", $arFilter = Array(), $arOrderFilter = Array(), $limit = 0)
	{
		...
		$strSql = "SELECT ";
		if(IntVal($limit) > 0)
			$strSql .= " TOP ".IntVal($limit);

		// if($byQuantity)
			// $strSql .= " b.PRODUCT_ID, b.CATALOG_XML_ID, b.PRODUCT_XML_ID, SUM(b.QUANTITY) as QUANTITY \n";
		// else
		...
		$strSql .= " GROUP BY b.PRODUCT_ID, b.CATALOG_XML_ID, b.PRODUCT_XML_ID, b.CURRENCY, b.NAME \n";
		if($byQuantity)
...
class CSaleViewedProduct extends CAllSaleViewedProduct
{
	...
	*/
	...
	public function GetList($arOrder = array("ID"=>"DESC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		$strSql = "SELECT /*TOP*/ ".$arSqls["SELECT"]." FROM b_sale_viewed_product V ";
	...
	*/
	public function _ClearViewed()
	{
		...
		$strSql =
			...
			"WHERE datediff(day, DATE_VISIT, getdate()) > ".$viewed_time;
	...
	*/
	public function DeleteForUser($FUSER_ID, $LIMIT = NULL)
	{
		...
		if (!empty($LIMIT) && IntVal($LIMIT) > 0)
			$strSqlLimit = " AND ID IN (SELECT TOP ".IntVal($LIMIT)." [ID] FROM b_sale_viewed_product ORDER BY [DATE_VISIT] DESC)";

		$strSql = "DELETE FROM b_sale_viewed_product WHERE FUSER_ID = '".$FUSER_ID."' ".$strSqlLimit;
