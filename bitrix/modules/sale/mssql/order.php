...
class CSaleOrder extends CAllSaleOrder
{
	...
	public static function GetList($arOrder = array("ID"=>"DESC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $arOptions = array())
	{
		...
		$arFields = array(
			...
			"LOCK_STATUS" => array("FIELD" => "case when DATE_LOCK is null then 'green' else case when dateadd(minute, ".$maxLock.", DATE_LOCK)<getdate() then 'green' else case when LOCKED_BY=".$userID." then 'yellow' else 'red' end end end", "TYPE" => "string"),
	...
	function GetLockStatus($ID, &$lockedBY, &$dateLock)
	{
		...
		$strSql =
			...
			"		else ".
			"			case ".
			"				when dateadd(minute, ".$maxLock.", DATE_LOCK)<getdate() then 'green' ".
			"				else ".
			"					case ".
