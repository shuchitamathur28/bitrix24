	...
	*/
	function _ClearProductSubscribe($LID)
	{
		...
		$strSql =
			"DELETE ".
			"FROM b_sale_basket ".
			"WHERE ((ORDER_ID IS NULL) OR (ORDER_ID = 0)) AND CAN_BUY = 'N' AND SUBSCRIBE = 'Y' AND datediff(day, DATE_INSERT, getdate()) > $dayDelete";
	...
	function Add($arFields)
	{
		...
		if(!$bFound)
		{
			...
			if ($isOrderConverted != "Y")
			{
				...
				{
					$IDENTITY_INSERT = "ON";
					$DB->Query("SET IDENTITY_INSERT b_sale_basket ON", false, "File: ".__FILE__."<br>Line: ".__LINE__);
				}
				...
				if ($boolProps)
				{
					foreach ($arFields["PROPS"] as &$prop)
					{
						if ('' != $prop["NAME"])
						{
							...
							{
								$IDENTITY_INSERT = "ON";
								$DB->Query("SET IDENTITY_INSERT b_sale_basket_props ON", false, "File: ".__FILE__."<br>Line: ".__LINE__);
							}
	...
	function GetLeave($arOrder = Array(), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = Array())
	{
		...
		$arSqls = CSaleOrder::PrepareSql($arFields, $arOrder, $arFilter, false, $arSelectFields);

		$arSqls["SELECT"] = str_replace("%%_DISTINCT_%%", "", $arSqls["SELECT"]);
		$groupByFields = " GROUP BY B.FUSER_ID, F.USER_ID, B.LID, B.CURRENCY, U.NAME, U.LAST_NAME, U.LOGIN, U.EMAIL";
...
class CSaleUser extends CAllSaleUser
{
	...
	function _Add($arFields)
	{
		...
		{
			$IDENTITY_INSERT = "ON";
			$DB->Query("SET IDENTITY_INSERT b_sale_fuser ON", false, "File: ".__FILE__."<br>Line: ".__LINE__);
		}
		...
		{
			$IDENTITY_INSERT="OFF";
			$DB->Query("SET IDENTITY_INSERT b_sale_fuser OFF", false, "File: ".__FILE__."<br>Line: ".__LINE__);
		}
	...
	function DeleteOld($nDays)
	{
		...
		$nDays = IntVal($nDays);
		$strSql = "SELECT TOP 300 ".
			" f.ID  ".
			"FROM b_sale_fuser f ".
			"LEFT JOIN b_sale_order o ON (o.USER_ID = f.USER_ID) ".
			"WHERE  ".
			"	datediff(day, f.DATE_UPDATE, getdate())>".$nDays.
			"	AND o.ID is null ".
	...
	function GetBuyersList($arOrder = Array(), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = Array())
	{
		...
		$grp = "";
		foreach($arSelectFields as $v)
		{
			if(!in_array($v, array("ORDER_SUM", "ORDER_COUNT", "LAST_ORDER_DATE")))
				$grp .= $arFields[$v]["FIELD"].", ";
		}
		$grp = substr($grp, 0, strlen($grp)-2);
