...
class CSaleOrderPropsValue extends CAllSaleOrderPropsValue
{
	...
	function Add($arFields)
	{
		...
		{
			$IDENTITY_INSERT = "ON";
			$DB->Query("SET IDENTITY_INSERT b_sale_order_props_value ON", false, "File: ".__FILE__."<br>Line: ".__LINE__);
		}
		...
		{
			$IDENTITY_INSERT="OFF";
			$DB->Query("SET IDENTITY_INSERT b_sale_order_props_value OFF", false, "File: ".__FILE__."<br>Line: ".__LINE__);
		}
	...
	function GetOrderProps($ORDER_ID)
	{
		...
		$strSql = "
			SELECT
				...
				PV.ORDER_PROPS_ID,
				PV.NAME,
				".self::getPropertyValueFieldSelectSql().",
				PV.CODE,
				...
				P.UTIL
			FROM
				b_sale_order_props_value PV
			LEFT JOIN b_sale_order_props P ON (P.ID = PV.ORDER_PROPS_ID)
			LEFT JOIN b_sale_order_props_group PG ON (PG.ID = P.PROPS_GROUP_ID)
			".self::getLocationTableJoinSql()."
			WHERE
				PV.ORDER_ID = $ORDER_ID
			ORDER BY
				PG.SORT, PG.NAME, P.SORT, P.NAME, P.ID
			";
