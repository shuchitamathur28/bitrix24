...
class CSaleStatus extends CAllSaleStatus
{
	...
	function GetList($arOrder = array(), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		$strSql = "SELECT /*TOP*/ ".$arSqls["SELECT"]." FROM B_SALE_STATUS S ".$arSqls["FROM"];
		if (!empty($arSqls["WHERE"]))
			$strSql .= " WHERE ".$arSqls["WHERE"];
		if (!empty($arSqls["GROUPBY"]))
			$strSql .= " GROUP BY ".$arSqls["GROUPBY"];
	...
	function GetByID($ID, $strLang = LANGUAGE_ID)
	{
		...
		else
		{
			$strSql = "
				SELECT
					S.ID, S.SORT, SL.LID, SL.NAME, SL.DESCRIPTION
				FROM
					B_SALE_STATUS S
	...
	function Update($ID, $arFields)
	{
		...
		if (isset($arFields['LANG']) && is_array($arFields['LANG']))
		{
			...
			foreach ($arFields['LANG'] as $statusLang)
			{
				...
				$strSql =
					"UPDATE b_sale_status_lang SET ".$langUpdate." WHERE STATUS_ID = '".$DB->ForSql($ID)."' AND LID = '".$DB->ForSql($langInsertFields["LID"])."'; ".
					"IF (@@ROWCOUNT = 0 ) INSERT INTO b_sale_status_lang (".$langInsert[0].") VALUES (".$langInsert[1].");";
				$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
			}
			if (isset($statusLang))
				unset($statusLang);
	...
	function GetPermissionsList($arOrder = array(), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		$strSql = "SELECT /*TOP*/ ".$arSqls["SELECT"]." FROM B_SALE_STATUS2GROUP S ".$arSqls["FROM"];
		if (!empty($arSqls["WHERE"]))
			$strSql .= " WHERE ".$arSqls["WHERE"];
		if (!empty($arSqls["GROUPBY"]))
			$strSql .= " GROUP BY ".$arSqls["GROUPBY"];
