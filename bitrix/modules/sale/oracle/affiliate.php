...
class CSaleAffiliate extends CAllSaleAffiliate
{
	...
	function Add($arFields)
	{
		...
		$ID = IntVal($DB->NextID("SQ_SALE_AFFILIATE"));

		$strSql =
			"INSERT INTO b_sale_affiliate(ID, ".$arInsert[0].", TIMESTAMP_X) ".
			"VALUES(".$ID.", ".$arInsert[1].", ".$DB->GetNowFunction().")";

		$arBinds = Array();
		if (is_set($arFields, "AFF_DESCRIPTION"))
			$arBinds["AFF_DESCRIPTION"] = $arFields["AFF_DESCRIPTION"];
	...
	function Update($ID, $arFields)
	{
		...
		$strSql = "UPDATE b_sale_affiliate SET ".$strUpdate.", TIMESTAMP_X = ".$DB->GetNowFunction()." WHERE ID = ".$ID." ";
