	...
	public function Add($arFields)
	{
		...
		if (!$arItems = $db_res->Fetch())//insert
		{
			if (\Bitrix\Main\Loader::includeModule('catalog'))
			{
				...
				$ID = IntVal($DB->NextID("sq_sale_viewed_product"));

				$strSql = "INSERT INTO b_sale_viewed_product (ID, ".$arInsert[0].", DATE_VISIT) VALUES(".$ID.", ".$arInsert[1].", ".$DB->GetNowFunction().")";
	...
	*/
	public function _ClearViewed()
	{
		...
		$strSql =
			...
			"WHERE DATE_VISIT < (SYSDATE-".$viewed_time.") AND ROWNUM <= 1000";
