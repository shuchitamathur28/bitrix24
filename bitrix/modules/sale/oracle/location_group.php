...
class CSaleLocationGroup extends CAllSaleLocationGroup
{
	function GetList($arOrder = Array("NAME"=>"ASC"), $arFilter=Array(), $strLang = LANGUAGE_ID)
	{
		...
		$strSql =
			...
			"FROM b_sale_location_group LG, b_sale_location_group_lang LGL ".
			"	".$strSqlSearchFrom." ".
			"WHERE LG.ID = LGL.LOCATION_GROUP_ID(+) AND LGL.LID(+) = '".$DB->ForSql($strLang, 2)."' ".
			"	".$strSqlSearch." ";
	...
	function GetByID($ID, $strLang = LANGUAGE_ID)
	{
		...
		$strSql =
			...
			"FROM b_sale_location_group LG, b_sale_location_group_lang LGL ".
			"WHERE LG.ID = ".$ID." ".
			"	AND LG.ID = LGL.LOCATION_GROUP_ID(+) AND LGL.LID(+) = '".$DB->ForSql($strLang, 2)."' ";
		$db_res = $DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
	...
	function Add($arFields)
	{
		...
		$ID = IntVal($DB->NextID("sq_b_sale_location_group"));

		$strSql =
			"INSERT INTO b_sale_location_group(ID, ".$arInsert[0].") ".
