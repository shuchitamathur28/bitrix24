...
class CSaleGroupAccessToSite extends CAllSaleGroupAccessToSite
{
	...
	function Add($arFields)
	{
		...
		$ID = IntVal($DB->NextID("SQ_SALE_SITE2GROUP"));

		$arInsert = $DB->PrepareInsert("b_sale_site2group", $arFields);

		$strSql =
			"INSERT INTO b_sale_site2group(ID, ".$arInsert[0].") ".
...
class CSaleGroupAccessToFlag extends CAllSaleGroupAccessToFlag
{
	...
	function Add($arFields)
	{
		...
		$ID = IntVal($DB->NextID("SQ_SALE_ORDER_FLAGS2GROUP"));

		$arInsert = $DB->PrepareInsert("b_sale_order_flags2group", $arFields);

		$strSql =
			"INSERT INTO b_sale_order_flags2group(ID, ".$arInsert[0].") ".
			"VALUES(".$ID.", ".$arInsert[1].")";
		$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
