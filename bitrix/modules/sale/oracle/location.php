...
class CSaleLocation extends CAllSaleLocation
{
	...
	public static function GetByID($ID, $strLang = LANGUAGE_ID)
	{
		...
		$strSql =
			...
			"	DECODE(LCL.ID, NULL, LC.NAME, LCL.NAME) as COUNTRY_NAME, ".
			"	DECODE(LGL.ID, NULL, LG.NAME, LGL.NAME) as CITY_NAME ".
			"FROM b_sale_location L, b_sale_location_country LC, b_sale_location_city LG, ".
			"	b_sale_location_country_lang LCL, b_sale_location_city_lang LGL ".
			"WHERE L.COUNTRY_ID = LC.ID ".
			"	AND L.CITY_ID = LG.ID(+) ".
			"	AND LC.ID = LCL.COUNTRY_ID(+) AND LCL.LID(+) = '".$DB->ForSql($strLang, 2)."' ".
			"	AND LG.ID = LGL.CITY_ID(+) AND LGL.LID(+) = '".$DB->ForSql($strLang, 2)."' ".
			"	AND L.ID = ".$ID." ";
		*/
		$strSql =
			"SELECT L.ID, L.COUNTRY_ID, L.CITY_ID, L.SORT, ".
			...
			"	DECODE(LCL.ID, NULL, LC.NAME, LCL.NAME) as COUNTRY_NAME, ".
			"	DECODE(LGL.ID, NULL, LG.NAME, LGL.NAME) as CITY_NAME, ".
			"	DECODE(LRL.ID, NULL, LR.NAME, LRL.NAME) as REGION_NAME ".
			"FROM b_sale_location L, b_sale_location_country LC, b_sale_location_city LG, ".
			"	b_sale_location_country_lang LCL, b_sale_location_city_lang LGL, b_sale_location_region LR, b_sale_location_region_lang LRL ".
			"WHERE L.COUNTRY_ID = LC.ID ".
			"	AND L.CITY_ID = LG.ID(+) ".
			"	AND LC.ID = LCL.COUNTRY_ID(+) AND LCL.LID(+) = '".$DB->ForSql($strLang, 2)."' ".
			"	AND LG.ID = LGL.CITY_ID(+) AND LGL.LID(+) = '".$DB->ForSql($strLang, 2)."' ".
			"	AND L.REGION_ID = LR.ID(+) ".
			"	AND LR.ID = LRL.REGION_ID(+) AND LRL.LID(+) = '".$DB->ForSql($strLang, 2)."' ".
			"	AND L.ID = ".$ID." ";
	...
	public static function GetCountryList($arOrder = Array("NAME_LANG"=>"ASC"), $arFilter=Array(), $strLang = LANGUAGE_ID)
	{
		...
		$strSql =
			...
			"	DECODE(CL.ID, NULL, C.NAME, CL.NAME) as NAME_LANG ".
			"FROM b_sale_location_country C, b_sale_location_country_lang CL ".
			"WHERE C.ID = CL.COUNTRY_ID(+) AND CL.LID(+) = '".$DB->ForSql($strLang, 2)."' ".
			"	".$strSqlSearch." ";
	...
	*/
	...
	public static function AddCountry($arFields)
	{
		...
		$ID = IntVal($DB->NextID("sq_sale_location_country"));

		$strSql =
			"INSERT INTO b_sale_location_country(ID, ".$arInsert[0].") ".
	...
	public static function AddCity($arFields)
	{
		...
		$ID = IntVal($DB->NextID("sq_sale_location_city"));

		$strSql =
			"INSERT INTO b_sale_location_city(ID, ".$arInsert[0].") ".
			"VALUES(".$ID.", ".$arInsert[1].")";
		$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
	...
	*/
	public static function AddRegion($arFields)
	{
		...
		$ID = IntVal($DB->NextID("sq_sale_location_region"));

		$strSql =
			"INSERT INTO b_sale_location_region(ID, ".$arInsert[0].") ".
			"VALUES(".$ID.", ".$arInsert[1].")";
		$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
	...
	public static function AddLocation($arFields)
	{
		...
		$ID = IntVal($DB->NextID("sq_b_sale_location"));

		$strSql =
			"INSERT INTO b_sale_location(ID, ".$arInsert[0].") ".
			"VALUES(".$ID.", ".$arInsert[1].")";
		$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
