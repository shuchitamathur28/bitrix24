...
class CSaleStatus extends CAllSaleStatus
{
	...
	function GetByID($ID, $strLang = LANGUAGE_ID)
	{
		...
		{
			$strSql =
				"SELECT S.ID, S.SORT, SL.LID, SL.NAME, SL.DESCRIPTION FROM B_SALE_STATUS S, B_SALE_STATUS_LANG SL WHERE ID = '".$ID."' ".
				"	AND S.ID = SL.STATUS_ID(+) AND SL.LID(+) = '".$strLang."' ";
	...
	function Update($ID, $arFields)
	{
		...
		if (isset($arFields['LANG']) && is_array($arFields['LANG']))
		{
			...
			foreach ($arFields['LANG'] as $statusLang)
			{
				...
				$langInsert = $DB->PrepareInsert("b_sale_status_lang", $langInsertFields);
				$langUpdate = $DB->PrepareUpdate("b_sale_status_lang", $langUpdateFields);
				$strSourceFields = "'".$DB->ForSql($ID)."' AS STATUS_ID, '".$DB->ForSql($langInsertFields["LID"])."' AS LID";
				$strUpdateFields = "";
				foreach ($langUpdateFields as $fieldName => $fieldValue)
				{
					$strSourceFields .= ", '".$DB->ForSql($fieldValue)."' AS ".$fieldName;
					$strUpdateFields .= ((!empty($strUpdateFields) ? ", " : ""))."target.".$fieldName." = source.".$fieldName;
				}
				if ($strUpdateFields == "")
					$strUpdateFields = "target.NAME = target.NAME";
				$strSql =
					"MERGE INTO b_sale_status_lang target ".
					"USING (SELECT ".$strSourceFields." FROM dual) source ".
					"ON (source.STATUS_ID = target.STATUS_ID AND source.LID = target.LID) ".
					"WHEN MATCHED THEN UPDATE SET ".$strUpdateFields." ".
					"WHEN NOT MATCHED ".
					"THEN INSERT (".$langInsert[0].") VALUES (".$langInsert[1].")";
				$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
			}
			if (isset($statusLang))
				unset($statusLang);
