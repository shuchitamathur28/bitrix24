...
class CSaleOrder extends CAllSaleOrder
{
	function Add($arFields)
	{
		...
		else
		{
			...
			$ID = IntVal($DB->NextID("SQ_B_SALE_ORDER"));
			...
			$strSql =
				"INSERT INTO b_sale_order(ID, ".$arInsert[0].") ".
				"VALUES(".$ID.", ".$arInsert[1].")";

			$arBinds = Array();
			if (is_set($arFields, "COMMENTS"))
				$arBinds["COMMENTS"] = $arFields["COMMENTS"];
	...
	function Update($ID, $arFields, $bDateUpdate = true)
	{
		...
		else
		{
			...
			if (is_set($arFields, "COMMENTS"))
				$arBinds["COMMENTS"] = $arFields["COMMENTS"];
			$updated = $DB->QueryBind($strSql, $arBinds, true);
	...
	public static function GetList($arOrder = array("ID"=>"DESC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $arOptions = array())
	{
		...
		$arFields = array(
			...
			"LOCK_STATUS" => array("FIELD" => "DECODE(DATE_LOCK, NULL, 'green', DECODE(SIGN(1440*(SYSDATE-DATE_LOCK)-".$maxLock."), 1, 'green', decode(LOCKED_BY,".$userID.",'yellow','red')))", "TYPE" => "string"),
			"LOCK_USER_NAME" => array("FIELD" => "'('||UL.LOGIN||') '||UL.NAME||' '||UL.LAST_NAME", "FROM" => "LEFT JOIN b_user UL ON (O.LOCKED_BY = UL.ID)", "TYPE" => "string"),
	...
	function GetLockStatus($ID, &$lockedBY, &$dateLock)
	{
		...
		$strSql =
			...
			"	DECODE(DATE_LOCK, NULL, 'green', ".
			"		DECODE(SIGN(1440*(SYSDATE-DATE_LOCK)-".$maxLock."), 1, 'green', ".
			"			decode(LOCKED_BY,".$userID.",'yellow','red')))	LOCK_STATUS ".
			"FROM b_sale_order ".
			"WHERE ID = ".$ID." ";
	...
	public function AddOrderHistory($OldFields, $NewFields)
	{
		...
		if ($bChange)
		{
			...
			$ID = IntVal($DB->NextID("SQ_SALE_ORDER_HISTORY"));

			$strSql = "INSERT INTO b_sale_order_history (ID, ".$arPrepare[0].") "."VALUES (".$ID.", ".$arPrepare[1].")";
