...
class CSaleOrderPropsValue extends CAllSaleOrderPropsValue
{
	...
	function Add($arFields)
	{
		...
		$ID = IntVal($DB->NextID("sq_b_sale_order_props_value"));

		$strSql =
			"INSERT INTO b_sale_order_props_value(ID, ".$arInsert[0].") ".
	...
	function GetOrderProps($ORDER_ID)
	{
		...
		$strSql =
			...
			"FROM b_sale_order_props_value PV, b_sale_order_props P, b_sale_order_props_group PG ".
			self::getLocationTableJoinSql().
			"WHERE PV.ORDER_ID = ".$ORDER_ID." ".
			"	AND PV.ORDER_PROPS_ID = P.ID(+) ".
			"	AND P.PROPS_GROUP_ID = PG.ID(+) ".
	...
	function GetOrderRelatedProps($ORDER_ID, $arFilter = array())
	{
		...
		$strSql =
			...
			"WHERE PV.ORDER_ID = ".$ORDER_ID." ".
			$strWhere.
			"ORDER BY PG.SORT, PG.NAME, PV.ID ";

		$db_res = $DB->Query($strSql, true, "File: ".__FILE__."<br>Line: ".__LINE__);
		return $db_res;
	}
}
?>