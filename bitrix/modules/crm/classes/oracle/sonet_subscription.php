...
class CCrmSonetSubscription extends CAllCrmSonetSubscription
{
	...
	public function Register($entityTypeID, $entityID, $typeID, $userID)
	{
		...
		$sql = "MERGE INTO {$tableName} USING (SELECT {$userID} USER_ID, '{$slEntityType}' SL_ENTITY_TYPE, {$entityID} ENTITY_ID, {$typeID} TYPE_ID FROM dual)
			source ON
			(
				source.USER_ID = {$tableName}.USER_ID
				AND source.SL_ENTITY_TYPE = {$tableName}.SL_ENTITY_TYPE
				AND source.ENTITY_ID = {$tableName}.ENTITY_ID
				AND source.TYPE_ID = {$tableName}.TYPE_ID
			)
			WHEN NOT MATCHED THEN
				INSERT (USER_ID, SL_ENTITY_TYPE, ENTITY_ID, TYPE_ID)
	...
	public function UnRegister($entityTypeID, $entityID, $typeID, $userID, $options = array())
	{
		...
		$tableName = self::TABLE_NAME;
		$slEntityType = $DB->ForSql(CCrmLiveFeedEntity::GetByEntityTypeID($entityTypeID));
		$deleteSql = "DELETE FROM {$tableName} WHERE USER_ID = $userID AND SL_ENTITY_TYPE = '{$slEntityType}' AND ENTITY_ID = {$entityID} AND TYPE_ID = $typeID";
