...
class CCrmActivity extends CAllCrmActivity
{
	...
	public static function DoSaveNearestUserActivity($arFields)
	{
		...
		$sql = "MERGE INTO B_CRM_USR_ACT USING (SELECT {$userID} USER_ID, {$ownerID} OWNER_ID, {$ownerTypeID} OWNER_TYPE_ID FROM dual)
			...
			WHEN MATCHED THEN
				UPDATE SET B_CRM_USR_ACT.ACTIVITY_TIME = {$activityTime}, B_CRM_USR_ACT.ACTIVITY_ID = {$activityID}, B_CRM_USR_ACT.SORT = '{$sort}'
			WHEN NOT MATCHED THEN
				INSERT (USER_ID, OWNER_ID, OWNER_TYPE_ID, ACTIVITY_TIME, ACTIVITY_ID, SORT, DEPARTMENT_ID)
