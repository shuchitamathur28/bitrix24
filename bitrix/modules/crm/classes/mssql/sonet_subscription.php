...
class CCrmSonetSubscription extends CAllCrmSonetSubscription
{
	...
	public function Register($entityTypeID, $entityID, $typeID, $userID)
	{
		...
		{
			$selectSql = "SELECT 'X' FROM {$tableName} WHERE USER_ID = {$userID} AND SL_ENTITY_TYPE = '{$slEntityType}' AND ENTITY_ID = {$entityID} AND TYPE_ID = {$typeID}";
			$dbResult = $DB->Query($selectSql, false, 'File: '.__FILE__.'<br/>Line: '.__LINE__);
