...
class CVote extends CAllVote
{
	...
	public static function GetPublicList($arFilter=Array(), $strSqlOrder="ORDER BY C.C_SORT, C.ID, V.DATE_START desc", $params = array())
	{
		...
		$strSql = "
			SELECT C.TITLE CHANNEL_TITLE, V.ID, V.CHANNEL_ID, V.C_SORT, V.ACTIVE, V.NOTIFY, V.AUTHOR_ID, V.TIMESTAMP_X,
				".$DB->DateToCharFunction("V.DATE_START")." DATE_START, ".$DB->DateToCharFunction("V.DATE_END")." DATE_END,
				V.URL, V.COUNTER, V.TITLE, V.DESCRIPTION, V.DESCRIPTION_TYPE, V.IMAGE_ID, V.EVENT1, V.EVENT2, V.EVENT3,
				V.UNIQUE_TYPE, V.KEEP_IP_SEC, V.KEEP_IP_SEC as DELAY, 'S' as DELAY_TYPE, V.TEMPLATE, V.RESULT_TEMPLATE,
		...
		else
		{
			if ($params["nTopCount"] > 0)
				$strSql = "SELECT * FROM (".$strSql.") WHERE ROWNUM<=".intval($params["nTopCount"]);
