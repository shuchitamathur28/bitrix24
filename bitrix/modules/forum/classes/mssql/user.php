...
class CForumUser extends CAllForumUser
{
	...
	public static function GetListEx($arOrder = Array("ID"=>"ASC"), $arFilter = Array())
	{
		...
		$arMainUserFields = array("LOGIN"=>"S", "NAME"=>"S", "LAST_NAME"=>"S", "SECOND_NAME"=>"S",
			...
			"PERSONAL_ZIP"=>"S", "PERSONAL_COUNTRY"=>"I", "PERSONAL_NOTES"=>"S", "EXTERNAL_AUTH_ID"=>"S", "WORK_COMPANY"=>"S",
	...
	public static function SearchUser($template, $arAddParams = array())
	{
		...
		$strSql =
			"SELECT /*TOP*/ U.ID, U.NAME, U.SECOND_NAME, U.LAST_NAME, U.LOGIN, F.SHOW_NAME,
				CASE
		...
		if (is_set($arAddParams, "bDescPageNumbering")) {
			...
			if ($arAddParams["nTopCount"] > 0)
				$strSql = str_replace("/*TOP*/", "TOP ".$arAddParams["nTopCount"], $strSql);
			$db_res = $DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
		}
	...
	public static function GetNameFieldsForQuery($sNameTemplate, $userTablePrefix = "U.")
	{
		...
		foreach($matches[0] as $val) {
			...
			$tmp[] = str_replace(
				...
				array(
					"CASE WHEN (".$userTablePrefix."NAME IS NULL OR LEN(".$userTablePrefix."NAME) <= 0) THEN '' ELSE ".$userTablePrefix."NAME END",
					"CASE WHEN (".$userTablePrefix."LAST_NAME IS NULL OR LEN(LTRIM(RTRIM(".$userTablePrefix."LAST_NAME))) <= 0) THEN '' ELSE ".$userTablePrefix."LAST_NAME + ',' END",
					"CASE WHEN (".$userTablePrefix."LAST_NAME IS NULL OR LEN(".$userTablePrefix."LAST_NAME) <= 0) THEN '' ELSE ".$userTablePrefix."LAST_NAME END",
					"CASE WHEN (".$userTablePrefix."SECOND_NAME IS NULL OR LEN(".$userTablePrefix."SECOND_NAME) <= 0) THEN '' ELSE ".$userTablePrefix."SECOND_NAME END",
					"CASE WHEN (".$userTablePrefix."NAME IS NULL OR LEN(LTRIM(RTRIM(".$userTablePrefix."NAME))) <= 0) THEN '' ELSE SUBSTRING(".$userTablePrefix."NAME,1,1)+'.' END",
					"CASE WHEN (".$userTablePrefix."SECOND_NAME IS NULL OR LEN(LTRIM(RTRIM(".$userTablePrefix."SECOND_NAME))) <= 0) THEN '' ELSE SUBSTRING(".$userTablePrefix."SECOND_NAME,1,1)+'.' END",
				),
				$val
			);
