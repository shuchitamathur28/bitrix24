...
/***************** /Event ******************************************/
	...
	public static function reindex(&$NS = array(), $oCallback = NULL, $callback_method = "")
	{
		...
		$strSql =
			"SELECT /*TOP*/ FT.ID as TID, FM.ID as MID,
		...
		if ($cnt > 0)
			$strSql = str_replace("/*TOP*/", "TOP ".$cnt, $strSql);
		...
		return
			"LTRIM(".
				"RTRIM(".
					"REPLACE(".
						"REPLACE(".
							"REPLACE((' ' + CONVERT(varchar(255), ISNULL(".implode(", '')) + '".$glue."' + CONVERT(varchar(255),ISNULL(", $pieces).", '')) + ' '), '".$glue.$glue."', '".$glue."'),".
						"'".$glue." ', ''),".
					"' ".$glue."', '')".
				")".
			")";
...
/***************** Event onBeforeGroupForumsAdd ********************/
		foreach (GetModuleEvents("forum", "onBeforeGroupForumsAdd", true) as $arEvent)
		{
			if (ExecuteModuleEventEx($arEvent, array(&$arFields)) === false)
				return false;
		}
