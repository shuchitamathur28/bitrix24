...
/***************** /Event ******************************************/
	...
	public static function reindex(&$NS = array(), $oCallback = NULL, $callback_method = "")
	{
		global $DB;
		...
		$strSql =
			"SELECT FT.ID as TID, FM.ID as MID,
				...
				FM.ID, FM.TOPIC_ID, FM.FORUM_ID, FM.USE_SMILES, FM.NEW_TOPIC,
				FM.APPROVED, FM.SOURCE_ID, 
				".$DB->DateToCharFunction("FM.POST_DATE", "FULL")." as POST_DATE, 
				FM.POST_MESSAGE, FM.POST_MESSAGE_HTML, FM.POST_MESSAGE_FILTER, FM.POST_MESSAGE_CHECK,
				FM.PARAM1, FM.PARAM2,
				FM.AUTHOR_ID, FM.AUTHOR_NAME, FM.AUTHOR_EMAIL, FM.AUTHOR_IP, FM.AUTHOR_REAL_IP, FM.GUEST_ID,
		...
		if ($cnt > 0)
			$strSql = "SELECT * FROM (".$strSql.") WHERE ROWNUM<".$cnt;
	...
	{
		return "TRIM(BOTH '".$glue."' FROM REPLACE((".implode("||'".$glue."'||", $pieces)."), '".$glue.$glue."', '".$glue."'))";
	}
}
