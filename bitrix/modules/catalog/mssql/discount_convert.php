...
class CCatalogDiscountConvertTmp
{
	...
	public static function CreateTable()
	{
		...
		$strSql = "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME LIKE '".$DB->ForSql(self::$strTableName)."'";
		...
		if (!($arResult = $dbResult->Fetch()))
		{
			...
			{
				$strSql = "ALTER TABLE ".self::$strTableName." ADD CONSTRAINT PK_".self::$strTableName." PRIMARY KEY (ID)";
				$result = $DB->Query($strSql, true);
	...
	public static function DropTable()
	{
		...
		if ($arResult = $dbResult->Fetch())
		{
			$strSql = "DROP TABLE ".self::$strTableName;
			$result = $DB->Query($strSql, true);
	...
	public static function GetLastID()
	{
		...
		$strSql = "SELECT TOP 1 ID FROM ".self::$strTableName." WHERE 1 = 1 ORDER BY ID DESC";
