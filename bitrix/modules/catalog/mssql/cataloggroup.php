...
class CCatalogGroup extends CAllCatalogGroup
{
	public static function GetByID($ID, $lang = LANGUAGE_ID)
	{
		...
		$groups = (CCatalog::IsUserExists() ? $USER->GetGroups() : '2');
		$strSql = "
			SELECT CG.ID, CG.NAME, CG.BASE, CG.SORT, CG.XML_ID, CGL.NAME as NAME_LANG,
				...
				case when MAX(CGG.ID) is null then 'N' else 'Y' end as CAN_ACCESS,
				case when MAX(CGG1.ID) is null then 'N' else 'Y' end as CAN_BUY
			FROM
				B_CATALOG_GROUP CG
			LEFT JOIN B_CATALOG_GROUP_LANG CGL ON (CGL.CATALOG_GROUP_ID=CG.ID and CGL.LANG='$lang')
			LEFT JOIN (
				SELECT ID, CATALOG_GROUP_ID FROM B_CATALOG_GROUP2GROUP
				WHERE GROUP_ID in ($groups) and BUY<>'Y'
			) CGG ON (CGG.CATALOG_GROUP_ID = CG.ID)
			LEFT JOIN (
				SELECT ID, CATALOG_GROUP_ID FROM B_CATALOG_GROUP2GROUP
				WHERE GROUP_ID in ($groups) and BUY = 'Y'
			) CGG1 ON (CGG1.CATALOG_GROUP_ID = CG.ID)
			WHERE
				CG.ID = $ID
	...
	public static function GetList($arOrder = array(), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		$arFields["CAN_ACCESS"] = array(
			"FIELD" => "case when MAX(CGG.ID) is null then 'N' else 'Y' end",
			"TYPE" => "char",
		...
		$arFields["CAN_BUY"] = array(
			"FIELD" => "case when MAX(CGG1.ID) is null then 'N' else 'Y' end",
			"TYPE" => "char",
		...
		$strSql = "SELECT /*TOP*/ ".$arSqls["SELECT"]." FROM B_CATALOG_GROUP CG ".$arSqls["FROM"];
		if (!empty($arSqls["WHERE"]))
			$strSql .= " WHERE ".$arSqls["WHERE"];
		if (!empty($arSqls["GROUPBY"]))
			$strSql .= " GROUP BY ".$arSqls["GROUPBY"];
