...
class CCatalogDiscountCoupon extends CAllCatalogDiscountCoupon
{
	...
	public static function GetList($arOrder = array(), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		$strSql = "SELECT /*TOP*/ ".$arSqls["SELECT"]." FROM B_CATALOG_DISCOUNT_COUPON CD ".$arSqls["FROM"];
		if (!empty($arSqls["WHERE"]))
			$strSql .= " WHERE ".$arSqls["WHERE"];
		if (!empty($arSqls["GROUPBY"]))
			$strSql .= " GROUP BY ".$arSqls["GROUPBY"];
	...
	* @see CCatalogDiscount::CouponOneOrderDisable
	*/
	...
	*/
	public static function IsExistCoupon($strCoupon)
	{
		...
		else
		{
			...
			$strSql = "select top 1 ID, COUPON from B_CATALOG_DISCOUNT_COUPON where COUPON='".$DB->ForSql($strCoupon)."'";
