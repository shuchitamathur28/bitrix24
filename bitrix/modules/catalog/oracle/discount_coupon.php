...
class CCatalogDiscountCoupon extends CAllCatalogDiscountCoupon
{
	public static function Add($arFields, $bAffectDataFile = true)
	{
		...
		$ID = (int)$DB->NextID('SQ_B_CATALOG_DISCOUNT_COUPON');
		...
		$strSql = "INSERT INTO B_CATALOG_DISCOUNT_COUPON(ID, ".$arInsert[0].") VALUES(".$ID.", ".$arInsert[1].")";
	...
	public static function Update($ID, $arFields)
	{
		...
		$discountIds = array();
		$arBinds = array();
		$strUpdate = $DB->PrepareUpdateBind("B_CATALOG_DISCOUNT_COUPON", $arFields, "", false, $arBinds);
		if (!empty($strUpdate))
		{
	...
	*/
	...
	public static function IsExistCoupon($strCoupon)
	{
		...
		else
		{
			...
			$strSql = "select * from (select ID, COUPON from B_CATALOG_DISCOUNT_COUPON where COUPON = '".$DB->ForSql($strCoupon)."') where ROWNUM<=1";
