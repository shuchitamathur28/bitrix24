...
class CCatalogDiscountSave extends CAllCatalogDiscountSave
{
	public static function Add($arFields, $boolCalc = false)
	{
		...
		$ID = (int)$DB->NextID('SQ_B_CATALOG_DISCOUNT');
		...
		{
			$boolBind = true;
			$arBinds['CONDITIONS'] = $arFields['CONDITIONS'];
			$arBinds['UNPACK'] = $arFields['UNPACK'];
		}

		$strSql = "insert into B_CATALOG_DISCOUNT(ID, ".$arInsert[0].") values(".$ID.", ".$arInsert[1].")";
	...
	public static function Update($intID, $arFields, $boolCalc = false)
	{
		...
		$arBinds = array();
		$strUpdate = $DB->PrepareUpdateBind("B_CATALOG_DISCOUNT", $arFields, "", false, $arBinds);
		if (!empty($strUpdate))
		{
			$strSql = "update B_CATALOG_DISCOUNT SET ".$strUpdate." WHERE ID = ".$intID." AND TYPE = ".self::ENTITY_ID;
			if (!empty($arBinds))
			{
				$arBindValues = array();
				foreach ($arBinds as $strKey)
					$arBindValues[$strKey] = $arFields[$strKey];
				$DB->QueryBind($strSql, $arBindValues, false, "File: ".__FILE__."<br>Line: ".__LINE__);
			}
			else
			{
				$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
	...
	public static function GetList($arOrder = array(), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		if (empty($arSelectFields))
			$arSelectFields = array('ID','XML_ID','SITE_ID','NAME','ACTIVE','SORT','CURRENCY','ACTIVE_FROM','ACTIVE_TO','COUNT_PERIOD','COUNT_SIZE','COUNT_TYPE','COUNT_FROM','COUNT_TO','ACTION_SIZE','ACTION_TYPE','TIMESTAMP_X','MODIFIED_BY','DATE_CREATE','CREATED_BY');
	...
	protected static function __UpdateUserInfoByDiscount($arParams, $arSettings = array())
	{
		...
		if (!empty($arParams) && is_array($arParams))
		{
			...
			if (0 < $intUserID && 0 < $intDiscountID && !empty($strActiveFrom) && !empty($strActiveTo))
			{
				if ($boolSearch)
				{
					$strQuery = 'select * from (select ID from B_CATALOG_DISC_SAVE_USER where DISCOUNT_ID = '.$intDiscountID.' AND USER_ID = '.$intUserID.') where rownum<=1';
