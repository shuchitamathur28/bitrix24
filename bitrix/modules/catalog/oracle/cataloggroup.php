...
class CCatalogGroup extends CAllCatalogGroup
{
	public static function GetByID($ID, $lang = LANGUAGE_ID)
	{
		...
		$strSql =
			"SELECT CG.ID, CG.NAME, CG.BASE, CG.SORT, CG.XML_ID, DECODE(MAX(CGG.ID), NULL, 'N', 'Y') as CAN_ACCESS, CGL.NAME as NAME_LANG, DECODE(MAX(CGG1.ID), NULL, 'N', 'Y') as CAN_BUY, ".
			...
			"FROM B_CATALOG_GROUP CG, B_CATALOG_GROUP_LANG CGL, ".
			"	(SELECT ID, CATALOG_GROUP_ID ".
			"	FROM B_CATALOG_GROUP2GROUP ".
			"	WHERE GROUP_ID IN (".$strUserGroups.") AND BUY <> 'Y') CGG, ".
			"	(SELECT ID, CATALOG_GROUP_ID ".
			"	FROM B_CATALOG_GROUP2GROUP ".
			"	WHERE GROUP_ID IN (".$strUserGroups.") AND BUY = 'Y') CGG1 ".
			"WHERE CG.ID = ".$ID." ".
			"	AND CG.ID = CGG.CATALOG_GROUP_ID(+) ".
			"	AND CG.ID = CGG1.CATALOG_GROUP_ID(+) ".
			"	AND CG.ID = CGL.CATALOG_GROUP_ID(+) ".
			"	AND CGL.LANG(+) = '".$DB->ForSql($lang)."' ".
			"GROUP BY CG.ID, CG.NAME, CG.BASE, CG.SORT, CG.XML_ID, CG.MODIFIED_BY, CG.CREATED_BY, CG.DATE_CREATE, CG.TIMESTAMP_X, CGL.NAME";
	...
	function Add($arFields)
	{
		...
		$groupID = (int)$DB->NextID("SQ_B_CATALOG_GROUP");

		$arInsert = $DB->PrepareInsert("B_CATALOG_GROUP", $arFields);
		$strSql = "INSERT INTO B_CATALOG_GROUP(ID, ".$arInsert[0].") VALUES(".$groupID.", ".$arInsert[1].")";
		$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
	...
	public static function GetList($arOrder = array(), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		$arFields = array(
			...
			"NAME_LANG" => array("FIELD" => "CGL.NAME", "TYPE" => "string", "FROM" => "LEFT JOIN B_CATALOG_GROUP_LANG CGL ON (CG.ID = CGL.CATALOG_GROUP_ID AND ".(strlen($arFilter["LID"]) <= 0 ? "(CGL.LANG = '' OR CGL.LANG is null)" : "CGL.LANG = '".$DB->ForSql($arFilter["LID"], 2)."'").")")
		);

		$arFields["CAN_ACCESS"] = array(
			"FIELD" => "DECODE(MAX(CGG.ID), NULL, 'N', 'Y')",
			"TYPE" => "char",
			"FROM" => "LEFT JOIN (SELECT ID, CATALOG_GROUP_ID FROM B_CATALOG_GROUP2GROUP WHERE GROUP_ID IN (".$strUserGroups.") AND BUY <> 'Y') CGG ON (CG.ID = CGG.CATALOG_GROUP_ID)",
			"GROUPED" => "Y"
		);
		$arFields["CAN_BUY"] = array(
			"FIELD" => "DECODE(MAX(CGG1.ID), NULL, 'N', 'Y')",
			"TYPE" => "char",
			"FROM" => "LEFT JOIN (SELECT ID, CATALOG_GROUP_ID FROM B_CATALOG_GROUP2GROUP WHERE GROUP_ID IN (".$strUserGroups.") AND BUY = 'Y') CGG1 ON (CG.ID = CGG1.CATALOG_GROUP_ID)",
			"GROUPED" => "Y"
		);
