	...
	function CalcActivity($arConfigs)
	{
		...
		if (isset($arConfigs['CONFIG']['ALL_POST_COEF']) && $arConfigs['CONFIG']['ALL_POST_COEF'] != 0) {
			$sqlAllPost = "
				...
				FROM b_blog_post
				WHERE DATE_PUBLISH  < dateadd(day, -30, getdate())
		...
		$strSql = "INSERT INTO b_rating_component_results (RATING_ID, MODULE_ID, RATING_TYPE, NAME, COMPLEX_NAME, ENTITY_ID, ENTITY_TYPE_ID, CURRENT_VALUE)
			...
			FROM (
				...
				SELECT
					AUTHOR_ID as ENTITY_ID,
					SUM(case when DATE_PUBLISH > dateadd(day, -1, getdate()) then 1 else 0 end)*".floatval($arConfigs['CONFIG']['TODAY_POST_COEF'])." +
					SUM(case when DATE_PUBLISH > dateadd(day, -7, getdate()) then 1 else 0 end)*".floatval($arConfigs['CONFIG']['WEEK_POST_COEF'])."+
				...
				SELECT
					AUTHOR_ID as ENTITY_ID,
					SUM(case when DATE_CREATE > dateadd(day, -1, getdate()) then 1 else 0 end)*".floatval($arConfigs['CONFIG']['TODAY_COMMENT_COEF'])." +
					SUM(case when DATE_CREATE > dateadd(day, -7, getdate()) then 1 else 0 end)*".floatval($arConfigs['CONFIG']['WEEK_COMMENT_COEF'])." +
