	...
	function CalcActivity($arConfigs)
	{
		...
		$strSql = "INSERT INTO b_rating_component_results (RATING_ID, MODULE_ID, RATING_TYPE, NAME, COMPLEX_NAME, ENTITY_ID, ENTITY_TYPE_ID, CURRENT_VALUE)
			...
			FROM (
				...
				SELECT
					AUTHOR_ID as ENTITY_ID,
					SUM(CASE WHEN DATE_PUBLISH>SYSDATE-1 THEN 1 ELSE 0 END)*".floatval($arConfigs['CONFIG']['TODAY_POST_COEF'])." +
					SUM(CASE WHEN DATE_PUBLISH>SYSDATE-7 THEN 1 ELSE 0 END)*".floatval($arConfigs['CONFIG']['WEEK_POST_COEF'])."+
				...
				SELECT
					AUTHOR_ID as ENTITY_ID,
					SUM(CASE WHEN DATE_CREATE>SYSDATE-1 THEN 1 ELSE 0 END)*".floatval($arConfigs['CONFIG']['TODAY_COMMENT_COEF'])." +
					SUM(CASE WHEN DATE_CREATE>SYSDATE-1 THEN 1 ELSE 0 END)*".floatval($arConfigs['CONFIG']['WEEK_COMMENT_COEF'])." +
