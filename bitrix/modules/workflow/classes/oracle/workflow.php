...
class CWorkflow extends CAllWorkflow
{
	...
	public static function Insert($arFields)
	{
		...
		$DB->QueryBind("
			...
			(ID, DATE_MODIFY, DATE_ENTER, ".$arInsert[0].")
			VALUES
			(".$ID.", sysdate, sysdate, ".$arInsert[1].")",
		$arBinds);
	...
	public static function Update($arFields, $DOCUMENT_ID)
	{
		...
		{
			$arBinds = array(
				"PROLOG" => $arFields["PROLOG"],
				"EPILOG" => $arFields["EPILOG"],
				"BODY" => $arFields["BODY"],
			);
			$DB->QueryBind("
				UPDATE b_workflow_document
				SET ".$strUpdate.", DATE_MODIFY=sysdate, DATE_ENTER=sysdate
