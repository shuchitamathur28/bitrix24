	...
	function voteCheck($arConfigs)
	{
		...
		$strSql = "
			...
			SET
				...
				USER_ID = 0
			WHERE 
				ENTITY_TYPE_ID = 'USER' and CREATED < dateadd(day, -".intval($arConfigs['CONDITION_CONFIG']['VOTE']['VOTE_LIMIT']).", getdate())
		";
		...
		$strSql = "
			...
			WHERE 
				RV.CREATED > dateadd(day, -".intval($arConfigs['CONDITION_CONFIG']['VOTE']['VOTE_LIMIT']).", getdate())
			and RV.VALUE > 0 and RV2.VALUE IS NULL and RV.OWNER_ID > 0
			GROUP BY RV.USER_ID, RV.OWNER_ID $sValue2
			HAVING 
				SUM(case
		...
		$strSql = "
			...
			SELECT 
				RV.ENTITY_TYPE_ID, 
				".$DB->IsNull('RV.ENTITY_ID', '0')." ENTITY_ID,
				'Y' as ACTIVE,
				".$DB->GetNowFunction()."  as CREATED,
			...
			WHERE 
				RATING_VOTING_ID = 0
			and RV.CREATED > dateadd(day, -".intval($arConfigs['CONDITION_CONFIG']['VOTE']['VOTE_LIMIT']).", getdate())
		...
		$strSql = "
			UPDATE
				b_rating_vote
			SET
				b_rating_vote.RATING_VOTING_ID = b_rating_voting.ID,
				b_rating_vote.ACTIVE = 'Y'
			FROM
				b_rating_voting
			WHERE   
				b_rating_vote.ENTITY_TYPE_ID = b_rating_voting.ENTITY_TYPE_ID
			and b_rating_vote.ENTITY_ID = b_rating_voting.ENTITY_ID
			and b_rating_vote.RATING_VOTING_ID = 0";
		$DB->Query($strSql, false, $err_mess.__LINE__);
		...
		$strSql = "
			UPDATE
				b_rating_voting
			SET
				b_rating_voting.TOTAL_VALUE = b_rating_voting_prepare.TOTAL_VALUE,
				b_rating_voting.TOTAL_VOTES = b_rating_voting_prepare.TOTAL_VOTES,
				b_rating_voting.TOTAL_POSITIVE_VOTES = b_rating_voting_prepare.TOTAL_POSITIVE_VOTES,
				b_rating_voting.TOTAL_NEGATIVE_VOTES = b_rating_voting_prepare.TOTAL_NEGATIVE_VOTES
			FROM
				b_rating_voting_prepare
			WHERE   
				b_rating_voting.ID = b_rating_voting_prepare.RATING_VOTING_ID";				
		$DB->Query($strSql, false, $err_mess.__LINE__);	
		
		// 7 DELETE OLD POST
		$strSql = "DELETE FROM b_rating_vote WHERE ENTITY_TYPE_ID = 'USER' and CREATED < dateadd(day, -".intval($arConfigs['CONDITION_CONFIG']['VOTE']['VOTE_LIMIT']).", getdate())";
		$DB->Query($strSql, false, $err_mess.__LINE__);
		return true;
	}
}
