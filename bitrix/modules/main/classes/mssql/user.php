...
class CUser extends CAllUser
{
	...
	public static function GetList(&$by, &$order, $arFilter=Array(), $arParams = Array())
	{
		...
		foreach ($arOrder as $field => $dir)
		{
			...
			{
				$nowDateTime = "DATEADD(second, ".CTimeZone::GetOffset().", ".$DB->CurrentTimeFunction().")";
				$arSelectFields['CURRENT_BIRTHDAY'] = "case
					...
					when
						...
						<
							cast(year(".$nowDateTime.") as varchar(4)) + '-' +
							REPLICATE('0',2-LEN(month(".$nowDateTime."))) + CONVERT(varchar(2),month(".$nowDateTime.")) + '-' +
							REPLICATE('0',2-LEN(day(".$nowDateTime."))) + CONVERT(varchar(2),day(".$nowDateTime."))
					then
						'".($cur_year+1)."-'+
						REPLICATE('0',2-LEN(DATEPART(mm, U.PERSONAL_BIRTHDAY)))+CONVERT(varchar(2),DATEPART(mm, U.PERSONAL_BIRTHDAY))+'-'+
						REPLICATE('0',2-LEN(DATEPART(dd, U.PERSONAL_BIRTHDAY)))+CONVERT(varchar(2),DATEPART(dd, U.PERSONAL_BIRTHDAY))
					...
					case "!EXTERNAL_AUTH_ID":
						...
						elseif (!is_array($val))
						{
							if($val <> '')
								...
								$arSqlSearch[] = "(U.EXTERNAL_AUTH_ID IS NOT NULL AND LEN(U.EXTERNAL_AUTH_ID) > 0)";
						}
						break;
					case "XML_ID":
