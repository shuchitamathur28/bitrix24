	...
	function voteCheck($arConfigs)
	{
		...
		$strSql = "
			...
			SET
				...
				USER_ID = 0
			WHERE 
				ENTITY_TYPE_ID = 'USER' and CREATED < SYSDATE-".intval($arConfigs['CONDITION_CONFIG']['VOTE']['VOTE_LIMIT'])."
		";
		...
		$strSql = "
			...
			WHERE 
				RV.CREATED > SYSDATE-".intval($arConfigs['CONDITION_CONFIG']['VOTE']['VOTE_LIMIT'])."
		...
		$strSql = "
			...
			WHERE 
				RATING_VOTING_ID = 0
			and RV.CREATED > SYSDATE-".intval($arConfigs['CONDITION_CONFIG']['VOTE']['VOTE_LIMIT'])."
		...
		$strSql = "
			...
			SET
				RV.RATING_VOTING_ID = (
					SELECT ID
					FROM b_rating_voting RVG
					WHERE RV.ENTITY_TYPE_ID = RVG.ENTITY_TYPE_ID	and RV.ENTITY_ID = RVG.ENTITY_ID
				),
				RV.ACTIVE = 'Y'
			WHERE
				RV.RATING_VOTING_ID = 0";
		$DB->Query($strSql, false, $err_mess.__LINE__);
		...
		$strSql = "
			...
			SET
				RVG.TOTAL_VALUE = (
					SELECT TOTAL_VALUE
					FROM b_rating_voting_prepare RVG0
					WHERE RVG0.RATING_VOTING_ID = RVG.ID
				),
				RVG.TOTAL_VOTES = (
					SELECT TOTAL_VOTES
					FROM b_rating_voting_prepare RVG0
				...
				RVG.TOTAL_POSITIVE_VOTES = (
					SELECT TOTAL_POSITIVE_VOTES
					FROM b_rating_voting_prepare RVG0
					WHERE RVG0.RATING_VOTING_ID = RVG.ID
				),
				RVG.TOTAL_NEGATIVE_VOTES = (
					SELECT TOTAL_NEGATIVE_VOTES
					FROM b_rating_voting_prepare RVG0
					WHERE RVG0.RATING_VOTING_ID = RVG.ID
				)
			WHERE
				RVG.ID IN (SELECT RATING_VOTING_ID FROM b_rating_voting_prepare RVG0)";				
		$DB->Query($strSql, false, $err_mess.__LINE__);	
		
		// 7 DELETE OLD POST
		$strSql = "DELETE FROM b_rating_vote WHERE ENTITY_TYPE_ID = 'USER' and CREATED < SYSDATE-".intval($arConfigs['CONDITION_CONFIG']['VOTE']['VOTE_LIMIT'])."";
		$DB->Query($strSql, false, $err_mess.__LINE__);
		return true;
	}
}
