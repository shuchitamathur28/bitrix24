...
class CUser extends CAllUser
{
	...
	public static function GetList(&$by, &$order, $arFilter=Array(), $arParams = Array())
	{
		...
		$online_interval = (array_key_exists("ONLINE_INTERVAL", $arParams) && intval($arParams["ONLINE_INTERVAL"]) > 0 ? intval($arParams["ONLINE_INTERVAL"]) : CUser::GetSecondsForLimitOnline());
				...
				case "!EXTERNAL_AUTH_ID":
					...
					elseif (!is_array($val))
					{
						if($val <> '')
							...
							$arSqlSearch[] = "U.EXTERNAL_AUTH_ID IS NOT NULL";
					}
					break;
			...
			if($field == "CURRENT_BIRTHDAY")
			{
				...
				$arSqlOrder[$field] = "CASE
					WHEN U.PERSONAL_BIRTHDAY IS NULL
						THEN '9999-99-99'
					WHEN '".$cur_year."'||TO_CHAR(U.PERSONAL_BIRTHDAY, '-MM-DD') < TO_CHAR(".$DB->CurrentTimeFunction()."+(".CTimeZone::GetOffset()."/86400), 'YYYY-MM-DD')
