	...
	function GetList($arOrder = Array("ID" => "DESC"), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $arParams = array())
	{
		...
		)
		{
			...
			$arFields["DATE_FOLLOW"] = Array(
				...
				END",
				"TYPE" => "datetime",
				"FROM" => "LEFT JOIN b_sonet_log_follow LFW ON LFW.USER_ID = ".$USER->GetID()." AND LFW.REF_ID = L.ID AND LFW.CODE = ".$DB->Concat("'L'", "CAST(L.ID as varchar(17))")
			);
...
//				$strSql = preg_replace("#^([\s]*select[\s]+)(.+)$#is", "SELECT TOP ".IntVal($arNavStartParams['nTopCount'])." \\2", $strSql);
	...
	{
		global $DB;
		return $DB->Query("DELETE SL FROM b_sonet_log AS SL WHERE EXISTS (SELECT BP.ID FROM b_blog_post BP WHERE BP.ID = SL.SOURCE_ID AND BP.BLOG_ID = ".intval($blog_id).") AND (SL.EVENT_ID = 'blog_post_micro' OR SL.EVENT_ID = 'blog_post')", true);
	}
}
