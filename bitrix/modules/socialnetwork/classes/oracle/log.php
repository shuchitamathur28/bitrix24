...
class CSocNetLog extends CAllSocNetLog
{
	...
	{
		global $DB;
		return $DB->Query("DELETE FROM b_sonet_log SL WHERE EXISTS (SELECT BP.ID FROM b_blog_post BP WHERE BP.ID = SL.SOURCE_ID AND BP.BLOG_ID = ".intval($blog_id).") AND (SL.EVENT_ID = 'blog_post_micro' OR SL.EVENT_ID = 'blog_post')", true);
	}
}
